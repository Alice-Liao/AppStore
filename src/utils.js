const domain = "https://folkloric-air-420802.ue.r.appspot.com"

// const newworkRequestStatus = fetch(`${domain}/login`), {
//     method: "",
//     headers: {
...
const handleResponseStatus = (response, errMsg) => {
    ...
    if (status === 401) { //没有登录 但是发请求 之前登录过
...
export const searchApps = (query) => {
    const title = query?.title ?? ""; // x === underfined ? "" : x
